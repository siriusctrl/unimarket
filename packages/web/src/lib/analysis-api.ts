...
export const createAnalysisApiClient = () => ({
  ...
  listDocuments: ({ market, reference }: { market: string; reference: string }) =>
    requestJson<{ documents: StoredChartAnalysis[] }>(
      `/api/analysis/documents?market=${encodeURIComponent(market)}&reference=${encodeURIComponent(reference)}&limit=20`,
    ),
